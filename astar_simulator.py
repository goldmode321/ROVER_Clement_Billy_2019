...
class Map:
    def __init__(self, name='maze1'):
        ...
        self.obs_dict = {'maze1':self.maze1}
        self.generate_map()
...
class AstarPathPlanning:
    def __init__(self, SharedVariable):
        ...
        self.node_calculated = dict() # Open set
        # self.calculated_path_dictionary = dict() # Open set
        self.node_use_for_calculation = dict() # Close set
        self.node_best_path = dict()


    # def available_node(self):
    #     '''find vaild region'''
    #     self.minx = round(min(self.SV.global_obstacle_x))
    #     self.miny = round(min(self.SV.global_obstacle_y))
    #     self.maxx = round(max(self.SV.global_obstacle_x))
    #     self.maxy = round(max(self.SV.global_obstacle_y))
    ...
    def calculate_cost(self, node):
        ...
        g_cost = self.SV.G_cost_factor * round(np.hypot(node.x - self.SV.start_x, node.y - self.SV.start_y), 2)
        # (Heuristic) Distance from node to end point
        h_cost = self.SV.H_cost_factor * round(np.hypot(node.x - self.SV.end_x, node.y - self.SV.end_y), 2)
        return g_cost + h_cost

    def node_invaild(self, node):
        # if np.min(np.hypot(self.SV.global_obstacle_x - node.x, self.SV.global_obstacle_y - node.y)) < \
        #     self.SV.rover_size + self.SV.obstacle_size:
        #     return True
        # else:
        #     return False
        return True if np.min(np.hypot(self.SV.global_obstacle_x - node.x, self.SV.global_obstacle_y - node.y)) < \
    ...
    def repeated_node(self, node):
        ...
        # if [node.x, node.y] in self.node_dictionary.values():
            # return True
        # else:
            # return False
    ...
    def planning(self):
        node = self.Node(
            ...
            self.SV.start_y,
            0,
            str(self.SV.start_x) + ',' + str(self.SV.start_y)
        ) # start point

        reach_target = False
        self.node_use_for_calculation[str(node.x) + ',' + str(node.y)] = node
        while not reach_target:
            # Open set, which is used for calculation, find lowest cost node for calculation
            node_id = min(self.node_use_for_calculation, key=lambda i: self.node_use_for_calculation[i].cost)
            node = self.node_use_for_calculation[node_id]
            if self.SV.show_progress:
                pass
            if node.x == self.SV.end_x and node.y == self.SV.end_y:
                print("Target reached")
                reach_target = True
                break

            # Remove this node from open_set
            del self.node_use_for_calculation[node_id]
            # Then add it to close_set, which is the one that already calculated
            self.node_calculated[node_id] = node

            # Node calculation
            for motion in self.motion:
                node.last_node_id = str(node.x) + ',' + str(node.y)
                node.x = node.x + motion[0]
                node.y = node.y + motion[1]
                if self.node_invaild(node):
                    continue
                if self.repeated_node(node):
                    continue
                node.cost = self.calculate_cost(node)
                if str(node.x) + ',' + str(node.y) not in self.node_use_for_calculation:
                    # Save it if it is a completely new node
                   self.node_use_for_calculation[str(node.x) + ',' + str(node.y)] = node
                else: # If not new node, see if it is the best path for now (lower cost)
                    if self.node_use_for_calculation[str(node.x) + ',' + str(node.y)].cost > node.cost:
                        self.node_use_for_calculation[str(node.x) + ',' + str(node.y)] = node
                    elif self.node_use_for_calculation[str(node.x) + ',' + str(node.y)].cost == node.cost:
                        print("unchanged")
        return self.calculate_path(node, self.node_calculated)
    ...
    def calculate_path(self, current_node, node_calculated):
        ...
        while last_node_id != [str(self.SV.start_x) + ',' + str(self.SV.start_y)]:
            node = node_calculated[last_node_id]
            route_x.append(node.x)
            route_y.append(node.y)
            last_node_id = node.last_node_id
...
class App:
    def __init__(self):
        ...
        self.route_plot = self.map_plot_widget.plot(pen=self.Pen.route_pen)
